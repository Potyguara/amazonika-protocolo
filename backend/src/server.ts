...
const allowedOrigins = [
  ...
  "https://sis.amazonikaengenharia.com.br",
  "http://localhost:5173",
  "http://127.0.0.1:5173",
...
app.use(
  cors({
    origin: (origin, callback) => {
      // Requisições sem Origin (curl, serviços internos etc.)
      if (!origin) {
...
async function generateSignedContractPdf(
  contractId: number
): Promise<{
  fileName: string;
  filePath: string;
  ...
  const clientSignature =
    contract.signatures.find(
      (signature) =>
        signature.signerRole === "CONTRATANTE"
    );
  ...
  if (
    ...
    throw new Error(
      "SIGNED_PDF_SIGNATURES_INCOMPLETE"
    );
  }
  ...
  const fileName =
    `${safeContractNumber}-assinado.pdf`;
  ...
  const methodLabel =
    (
      ...
      if (
        ...
        return "Código de confirmação (OTP) enviado por e-mail";
      }
      ...
      if (
        ...
        return "Certificado digital / ICP";
      }
  ...
  const doc =
    new PDFDocument({
      ...
      info: {
        ...
        Subject:
          "Contrato de prestação de serviços assinado eletronicamente",
      },
    });
  ...
  doc
    ...
    .text(
      "CERTIFICADO DE ASSINATURAS ELETRÔNICAS",
      {
        align:
          "center",
      }
  ...
  paragraph(
    "Este certificado integra o instrumento contratual e registra as evidências eletrônicas associadas às manifestações de vontade das partes."
  );
  ...
  labelValue(
    "Nome/Razão Social",
    contractorSignature.signerName
  );

  labelValue(
    "CPF/CNPJ",
    contractorSignature.signerCpfCnpj
  );

  labelValue(
    "E-mail",
    contractorSignature.signerEmail
  );

  labelValue(
    "Método",
    methodLabel(
      contractorSignature.method
    )
  );
  ...
  labelValue(
    "Data e hora",
    `${formatDateTime(
      contractorSignature.signedAt
    )} (America/Belem)`
  );

  labelValue(
    "IP registrado",
    contractorSignature.signerIp
  );
  ...
  doc
    ...
    .fontSize(9)
    .text(
      "Identificador criptográfico da assinatura:"
    );
  ...
  doc
    ...
    .text(
      contractorSignature.signatureHash,
      {
        width:
          contentWidth,
      }
  ...
  labelValue(
    "CPF/CNPJ",
    clientSignature.signerCpfCnpj
  );

  labelValue(
    "E-mail",
    clientSignature.signerEmail
  );

  labelValue(
    "Método",
    methodLabel(
      clientSignature.method
    )
  );
  ...
  doc
    ...
    .text(
      "SHA-256 do conteúdo contratual assinado:"
    );
  ...
  doc
    ...
    .text(
      "Documento gerado pelo SIS Amazonika a partir do conteúdo contratual e das evidências eletrônicas registradas no sistema.",
      {
        align:
          "center",
      }
  ...
  /*
   * Aguarda efetivamente a conclusão da escrita.
   */
  ...
  /*
   * Somente após o PDF existir integralmente
   * registramos seu caminho no contrato.
   */
...
/*
 * =========================================================
 * ASSINATURA INTERNA DA CONTRATADA
 * =========================================================
 *
 * A assinatura é realizada por usuário interno autenticado.
 * A identidade empresarial vem das configurações da empresa
 * e a identidade operacional fica vinculada ao req.user.
 */
app.post(
  "/contracts/:id/sign-contractor",
  authMiddleware,
  requireRoles(["GERENTE", "PROGRAMADOR"]),
  async (req: any, res) => {
    try {
      ...
      if (contract.status !== "GERADO") {
        return res.status(400).json({
          message:
            "Somente contratos no status GERADO podem ser assinados inicialmente pela CONTRATADA.",
        });
      }

      const existingContractorSignature =
        contract.signatures.find(
          (signature) =>
            signature.signerRole === "CONTRATADA"
        );

      if (existingContractorSignature) {
        return res.status(400).json({
          message:
            "Este contrato já possui assinatura eletrônica da CONTRATADA.",
        });
      }
      ...
      if (!req.user?.id || !req.user?.name || !req.user?.email) {
        return res.status(401).json({
          message:
            "Não foi possível identificar o usuário interno responsável pela assinatura.",
        });
      }
      ...
      if (!signerCpfCnpj) {
        return res.status(400).json({
          message:
            "O CNPJ da CONTRATADA não está configurado nas configurações da empresa.",
        });
      }
      ...
      if (!signerEmail) {
        return res.status(400).json({
          message:
            "O e-mail da CONTRATADA não está configurado.",
        });
      }
      ...
      const acceptedTermsText =
        "A CONTRATADA, por intermédio de usuário interno autenticado e autorizado, declara que conferiu o instrumento contratual e manifesta sua vontade de assiná-lo eletronicamente.";
        ...
        await prisma.$transaction(
          async (tx) => {
            ...
            if (
              ...
              throw new Error(
                "CONTRACT_NOT_AVAILABLE_FOR_CONTRACTOR_SIGNATURE"
              );
            }

            const alreadySigned =
              currentContract.signatures.some(
                (item) =>
                  item.signerRole ===
                  "CONTRATADA"
              );
            ...
            await tx.auditLog.create({
              data: {
                ...
                description:
                  `Contrato ${contract.contractNumber} assinado eletronicamente pela CONTRATADA.`,
      ...
      return res.status(201).json({
        message:
          "Contrato assinado eletronicamente pela CONTRATADA com sucesso.",
      ...
      if (
        ...
        return res.status(400).json({
          message:
            "O contrato não está mais disponível para assinatura da CONTRATADA.",
        });
      }
      ...
      return res.status(500).json({
        message:
          "Erro ao registrar assinatura eletrônica da CONTRATADA.",
      });
    }
  }
);
  ...
  async (req: any, res) => {
    try {
      ...
      if (!contractorSignature) {
        return res.status(400).json({
          message:
            "O contrato deve ser assinado eletronicamente pela CONTRATADA antes de ser enviado ao cliente.",
        });
      }
...
app.post("/public/contracts/:token/signature-otp", async (req, res) => {
  try {
    ...
    if (!contractorSignature) {
      return res.status(400).json({
        message:
          "O contrato ainda não foi assinado pela CONTRATADA.",
      });
    }
...
app.post("/public/contracts/:token/sign", async (req, res) => {
  try {
     ...
     * =====================================================
     *
     * A assinatura já foi concluída e persistida de forma
     * atômica. A geração do PDF ocorre somente após o commit
     * da transação.
     *
     * Eventual falha na geração do arquivo não invalida nem
     * desfaz as assinaturas eletrônicas já registradas.
     */
    ...
    try {
      ...
      console.log(
        `PDF final do contrato ${contract.contractNumber} gerado automaticamente: ${signedPdf.publicPath}`
      );
    } catch (pdfError) {
      console.error(
        `Contrato ${contract.contractNumber} foi assinado, mas houve falha ao gerar o PDF final:`,
        pdfError
      );
    }
    ...
    /*
     * =====================================================
     * ENVIO AUTOMÁTICO DO CONTRATO ASSINADO
     * =====================================================
     *
     * O PDF final somente é enviado depois de ter sido
     * gerado com sucesso.
     *
     * Falha no envio não invalida a assinatura eletrônica
     * nem remove o PDF já gerado.
     */
    ...
    if (signedPdf) {
      try {
        ...
        if (!emailTransporter) {
          throw new Error(
            "SMTP_NOT_CONFIGURED_FOR_SIGNED_CONTRACT"
          );
        }

        /*
         * Utilizamos prioritariamente o e-mail validado
         * pelo OTP. Como fallback, usamos o cadastro
         * original do cliente.
         */
        const signedContractRecipient =
          normalizedSignerEmail ||
          contract.client.email;

        if (!signedContractRecipient) {
          throw new Error(
            "SIGNED_CONTRACT_RECIPIENT_NOT_FOUND"
          );
        }

        const signedContractEmailHtml = `
          <div style="
          ...
          ">
            ...
            ">
              ...
              ">
                ...
                <p>
                  Prezado(a) ${normalizedSignerName},
                </p>
                ...
                <p>
                  O documento contratual completo,
                  acompanhado do certificado das
                  assinaturas eletrônicas e dos
                  identificadores de integridade,
                  segue anexo a este e-mail em
                  formato PDF.
                </p>

                <p>
                  Recomendamos manter este documento
                  para seus registros.
                </p>
        ...
        signedPdfEmail = {
          messageId:
            signedContractEmailInfo.messageId,

          accepted:
            signedContractEmailInfo.accepted,

          rejected:
            signedContractEmailInfo.rejected,
        };

        console.log(
          `PDF final do contrato ${contract.contractNumber} enviado automaticamente para ${signedContractRecipient}. Message-ID: ${signedContractEmailInfo.messageId}`
        );

        /*
         * Auditoria independente do envio.
         */
        await prisma.auditLog.create({
          data: {
            userId:
            ...
            description:
              `PDF final do contrato ${contract.contractNumber} enviado para ${signedContractRecipient}.`,
      ...
      } catch (signedContractEmailError) {
        console.error(
          `Contrato ${contract.contractNumber} foi assinado e o PDF final foi gerado, mas houve falha no envio automático por e-mail:`,
          signedContractEmailError
        );
      }
    }
